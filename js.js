...
$.ajax(url)
    .done(function(xml){
        $(xml).find("kombatente").each(function(){
            ...
            if(id === pos){
                $("#individual").append('<div class="card"><p class="procurado">Kombatente</p> <img class="foto" src="'+ $(this).find("id").text() +'.jpg"><p class="vivo">Vivo</p> <p class="nome">'+ $(this).find("nome").text() +'</p> <p>'+ $(this).find("nome_real").text() +'<\p> <p class="rec">Nível de Poder:'+ $(this).find("nivel_de_poder").text()+' </p><p> Status:'+ $(this).find("status").text()+' </p><\div>');
            }
        });
    })
    .fail(function(){
